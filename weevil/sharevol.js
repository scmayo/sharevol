...
scale:[getSearchVariable("dx",1),getSearchVariable("dy",1),getSearchVariable("dz",1)]},props.url&&loadTexture()):a="weevil.json");a&&($("status").innerHTML="Loading params...",ajaxReadFile(decodeURI(a),loadData,!0))}function loadStoredData(a){if(localStorage[a])try{props=JSON.parse(localStorage[a])}catch(b){alert("parse error: "+b.message),localStorage[a]=null}}
...
!0)))}},"Update"),a.add({Reset:function(){resetFromData(reset)}},"Reset"),a.add({Restore:function(){resetFromData(props)}},"Restore"),a.add({Export:function(){exportData()}},"Export"),a.add({loadFile:function(){document.getElementById("fileupload").click()}},"loadFile").name("Load Image file"),a.add({ColourMaps:function(){window.colourmaps.toggle()}},"ColourMaps"),slicer&&slicer.addGUI(a));window.onbeforeunload=saveData}function fileSelected(a){filesProcess(a)}
